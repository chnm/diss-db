...
// Lazy-mount an artboard's children when its card enters the viewport.
// Mounting 6+ full-page React trees synchronously locks the main thread for
// seconds; IO-deferred mount with a one-tick idle stagger keeps the canvas
// responsive even on a 18-artboard exploration. The placeholder is a flush
// transparent fill so containIntrinsicSize still holds layout.
function DCLazyChildren({ children }) {
  const [mounted, setMounted] = React.useState(false);
  const ref = React.useRef(null);
  React.useEffect(() => {
    if (mounted || !ref.current) return;
    // Generous rootMargin so panning into view doesn't show empty cards.
    const obs = new IntersectionObserver(([e]) => {
      if (!e.isIntersecting) return;
      obs.disconnect();
      // Stagger via idle callback so multiple newly-visible cards don't
      // mount in the same frame.
      const schedule = window.requestIdleCallback || ((fn) => setTimeout(fn, 0));
      schedule(() => setMounted(true), { timeout: 400 });
    }, { rootMargin: '400px' });
    ...
    <div ref={ref} data-dc-slot={id} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      <div ref={cardRef} className="dc-card"
        style={{ borderRadius: 2, boxShadow: '0 1px 3px rgba(0,0,0,.08),0 4px 16px rgba(0,0,0,.06)', overflow: 'hidden', width, height, background: '#fff',
          // Skip paint+layout for artboards scrolled out of view — keeps
          // large canvases (18+ full-page artboards) responsive.
          contentVisibility: 'auto', containIntrinsicSize: `${width}px ${height}px`,
          ...style }}>
        <DCLazyChildren>{children}</DCLazyChildren>
      </div>
    </div>
  );
}
